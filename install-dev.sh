#!/usr/bin/env bash
#
# Install the chiron CLI from the DEV channel.
#
...
#
#   curl -fsSL <RAW_URL>/install-dev.sh | bash -s -- --code CHIR-… --server <url>
#
# WHY A SEPARATE SCRIPT AND NOT A FLAG PEOPLE TYPE
# Prod users must not be able to reach the dev binary by accident, and the
...
echo "→ chiron CLI · DEV channel"
CHIRON_CHANNEL=dev bash "$tmp/install.sh" "$@"
